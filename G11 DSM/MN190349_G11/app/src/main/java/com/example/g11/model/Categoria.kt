package com.example.g11.model

import android.content.ContentValues
import android.content.Context
import android.database.Cursor
import android.database.sqlite.SQLiteDatabase
import com.example.g11.db.HelperDB
...
class Categoria (context: Context?){
    ...
    //Debido a que el Spinner solamente guarda el nombre, esta funcion nos ayuda a recuperar el ID de la categoria
    fun searchId(nombre: String) : Int? {
        val columns = arrayOf(COL_ID, COL_NOMBRE)
        var cursor: Cursor? = db!!.query(
            TABLE_NAME_CATEGORIA, columns, "$COL_NOMBRE=?", arrayOf(nombre.toString()),null,null,null
        )
        cursor!!.moveToFirst()
